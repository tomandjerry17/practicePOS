...
$(document).ready(function() {
    ...
    $('button.pos-express-finalize').click(function() {
        ...
        if (pay_method == 'card') {
            ...
            // For cash payments, show payment modal to get amount received
            // First set the payment method to cash and trigger change event
            var payment_method_dropdown = $('#payment_rows_div')
                .find('.payment_types_dropdown')
                .first();
            payment_method_dropdown.val('cash').trigger('change');
    ...
    // Handle payment modal submit button for cash payments
    $('button#pos-save').click(function() {
        // Validate amount received for cash payments
        var hasCashPayment = false;
        var amountReceivedValid = true;
        
        $('#payment_rows_div').find('.payment_types_dropdown').each(function() {
            if ($(this).val() == 'cash') {
                hasCashPayment = true;
                var payment_row = $(this).closest('.payment_row');
                var amount_received = payment_row.find('.amount-received').val();
                
                if (!amount_received || parseFloat(amount_received) <= 0) {
                    amountReceivedValid = false;
                    toastr.error('Please enter the amount received for cash payment');
                    return false;
                }
            }
        });
        
        if (!amountReceivedValid) {
            return false;
        }
            ...
            success: function(result) {
                if (result) {
                    ...
                    $(appended)
                        ...
                        .change()
                        .select();
                    __select2($(appended).find('.payment_row').find('.select2'));
    ...
    pos_form_validator = pos_form_obj.validate({
        submitHandler: function(form) {
            ...
            // Cash amount received validation is now handled in the payment modal
            // No need to duplicate validation here

            // Balance validation removed - not needed for POS as full payment is always collected
...
// Handle amount received input changes for change calculation
$(document).on('input', '.amount-received', function() {
    var payment_row = $(this).closest('.payment_row');
    var payment_amount = parseFloat(payment_row.find('.payment-amount').val()) || 0;
    var amount_received = parseFloat($(this).val()) || 0;
    
    if (amount_received > 0) {
        var change = amount_received - payment_amount;
        if (change >= 0) {
            // Update the change return field
            $('#change_return').val(change.toFixed(2));
            $('span.change_return_span').text(__currency_trans_from_en(change, true));
            $('#change_return_payment_data').removeClass('hide');
        } else {
            // If amount received is less than payment amount, clear change
            $('#change_return').val(0);
...
function calculate_balance_due() {
    ...
    $('span.total_paying').text(__currency_trans_from_en(total_paying, true));
            ...
            // Balance field removed - not needed for POS
...
function reset_pos_form(){
	...
	$('span.total_quantity, span.price_total, span#total_discount, span#order_tax, span#total_payable, span#shipping_charges_amount').text(0);
	        $('span.total_payable_span', 'span.total_paying').text(0);
...
$(document).on('change', '.payment_types_dropdown', function(e) {
    ...
    // Handle amount received field visibility for cash payments
    var amount_received_field = payment_row.find('.amount-received-field');
    if (payment_type == 'cash') {
        amount_received_field.removeClass('hide');
        // Auto-fill amount received with payment amount if empty
        var payment_amount = payment_row.find('.payment-amount').val();
        var amount_received = payment_row.find('.amount-received');
        if (amount_received.val() == '' && payment_amount != '') {
            amount_received.val(payment_amount);
        }
    } else {
        amount_received_field.addClass('hide');
        // Clear amount received when not cash
        payment_row.find('.amount-received').val('');
    }
