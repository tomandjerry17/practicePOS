...
class TransactionUtil extends Util
{
                ...
                if ($payment_amount != 0) {
                    ...
                    $payment_data = [
                        ...
                        'payment_for' => $transaction->contact_id,
                            ...
                            'amount_received' => isset($payment['amount_received']) ? ($uf_data ? $this->num_uf($payment['amount_received']) : $payment['amount_received']) : null,
                    ];
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        if ($transaction_type == 'sell' && $transaction->status == 'final') {
            // Default behavior uses change return (is_return=1) to reduce paid amount,
            // which causes confusion on receipts. For receipt display we want:
            //   total_paid = min(final_total, sum of positive payments)
            //   total_due  = max(0, final_total - sum of positive payments)
            $positive_paid_amount = 0;
            if (!empty($transaction->payment_lines)) {
                foreach ($transaction->payment_lines as $pl) {
                    if (empty($pl->is_return)) {
                        $positive_paid_amount += (float) $pl->amount;
                    }
                }
            }

            $effective_paid = min((float) $transaction->final_total, (float) $positive_paid_amount);
            $due = max(0.0, (float) $transaction->final_total - (float) $positive_paid_amount);

            $output['total_paid'] = ($effective_paid == 0) ? 0 : $this->num_f($effective_paid, $show_currency, $business_details);
                ...
                if (! empty($payments)) {
                    foreach ($payments as $value) {
                        ...
                        if ($value['method'] == 'cash') {
                            ...
                            // Add amount received and change for cash payments
                            if (!empty($value['amount_received']) && $value['amount_received'] > $value['amount']) {
                                $payment_data['cash_amount_received'] = $this->num_f($value['amount_received'], $show_currency, $business_details);
                                $change = $value['amount_received'] - $value['amount'];
                                $payment_data['cash_change'] = $this->num_f($change, $show_currency, $business_details);
                            }
                            
                            $output['payments'][] = $payment_data;
